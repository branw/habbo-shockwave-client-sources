...
on _MinigameTestChecksum me, i_iChecksum
  ...
  if i_iChecksum <> tMyChecksum then
    put "*** TURN" && m_rCurrentTurn.GetNumber() && " - CHECKSUM MISMATCH! server says:" && i_iChecksum & ", we say:" && tMyChecksum && "seed:" && m_rQuickRandom.IterateSeed(m_rCurrentTurn.GetNumber())
