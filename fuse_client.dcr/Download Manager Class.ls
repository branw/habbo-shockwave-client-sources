property ancestor, pTaskQueue, pActiveTasks, pReceivedTasks, pCompleteTasks, pTypeDefList, pDefaultURL
...
on getLoadPercent me, tMemNameOrNum
  ...
  if not voidp(pActiveTasks[tMemName]) then
    ...
    if not voidp(pTaskQueue[tMemName]) then
      ...
      if not voidp(pCompleteTasks[tMemName]) then
        return 1.0
      end if
    end if
  end if
...
on setProperty me, tPropID, tValue
  case tPropID of
    #defaultURL:
      return setDefaultURL(me, tValue)
  end case
  return 0
end

on solveNetErrorMsg me, tNetError
  case tNetError of
    4:
...
on recognizeMemberType me, tURL
  ...
  if not symbolp(tFileType) then
    return error(me, "Couldn't recognize member's type:" && tURL, #recognizeMemberType)
  else
    return tFileType
  end if
end
