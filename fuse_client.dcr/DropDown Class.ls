property pState, pProp, pCasts, pTextlist, pMenuItems, pDropMenuImg, pDropActiveBtnImg, pDropDownImg, pLineHeight, pMarginTop, pMarginBottom, pMarginLeft, pTextKeys, pAlignment, pBlend, pOpenDir, pMaxWidth, pDotLineImg, pFont, pFonSize, pNumberOfMenuItems, pSelectedItemNum, pRollOverItem, pLoc, pFixedSize, pOrigWidth, pLastRollOver, pTextWidth, pClickPass
...
on setSelection me, tSelNumOrStr
  if stringp(tSelNumOrStr) then
    tSelNumOrStr = pMenuItems.getPos(tSelNumOrStr)
  end if
  if tSelNumOrStr > 0 and tSelNumOrStr <= pNumberOfMenuItems then
    pSelectedItemNum = tSelNumOrStr
...
on mouseWithin me
  if pState = #open then
    ...
    if pRollOverItem <> pLastRollOver then
      ...
      tMemberDescription = pProp[#up][#members][#activeline]
      tMember = member(tMemberDescription[#member], pCasts[tMemberDescription[#cast]])
...
on UpdateImageObjects me, tPalette, tState
  ...
  if not voidp(pProp[#optionalimage]) then
    ...
    repeat with i = 1 to tOptionalImages.count()
      ...
      tMember = member(tMemName, pCasts[tDesc[#cast]])
      tImage = tMember.image.duplicate()
      if tImage.paletteRef <> tPalette then
        tImage.paletteRef = tPalette
      end if
...
on createDropImg me, tItemsList, tListOfAllItemsOrNot, tState
  ...
  tTextMember.text = s.line[1..s.line.count - 1]
  ...
  if pFixedSize = 1 then
    ...
    if tListOfAllItemsOrNot = 1 then
      ...
      tCharNum = 1
      tSofarChars = 0
      repeat with tLineN = 1 to s.line.count
        tSofarChars = tSofarChars + s.line[tLineN].char.count
        if s.line[tLineN].char.count > tMaxLengt then
  ...
  tdestrect = tMyTextimg.rect + rect(0, pMarginTop, 0, pMarginTop)
  ...
  tNewImg.copyPixels(tMyTextimg, tdestrect, tMyTextimg.rect)
  return tNewImg
end
