property ancestor, pClientID, pProcedures, pActive, pLock, pScaleFlag, pScaleOffset, pElemList, pMemberList, pGroupData, pSpecialIDList, pClientRect, pElemClsList, pWindowMngr
...
on construct me
  me.pSpriteList = [:]
  me.pBoundary = rect(0, 0, (the stage).rect.width, (the stage).rect.height) + [-20, -20, 20, 20]
  pActive = 0
  pLock = 0
  pScaleFlag = 0
  pClientID = VOID
...
on deconstruct me
  ...
  repeat with i = 1 to me.pSpriteList.count
    tSprNum = me.pSpriteList[i].spriteNum
...
on define me, tProps
  ...
  me.pLocZ = tProps[#locZ]
  me.pBoundary = tProps[#boundary]
...
on unmerge me
  ...
  repeat with tSprite in tGroupData[#sprites]
    me.pSpriteList.deleteProp(me.pSpriteList.getOne(tSprite))
...
on center me
  tX = (the stageRight - the stageLeft) / 2 - me.pWidth / 2
  tY = (the stageBottom - the stageTop) / 2 - me.pHeight / 2
...
on getClientRect me
  return rect(me.pLocX, me.pLocY, me.pLocX + me.pWidth, me.pLocY + me.pHeight)
end

on getSpriteByID me, tid
  tSprite = me.pSpriteList[tid]
...
on elementExists me, tid
  return not voidp(me.pElemList[tid])
end

on movePartBy me, ttype, tX, tY, tInverse
  tSprite = me.pSpriteList[ttype]
  if voidp(tSprite) then
    return 0
  end if
  if tInverse then
    repeat with i = 1 to me.pSpriteList.count
      tSymbol = me.pSpriteList.getPropAt(i)
      if tSymbol <> ttype then
        tSprite = me.pSpriteList[tSymbol]
...
on redirectEvent me, tEvent, tSprID
  ...
  if tClient <> 0 then
    return call(tMethod, tClient, tEvent, tSprID, tParam)
  else
...
on buildVisual me, tLayout
  ...
  repeat with tElement in tLayout[#elements]
    ...
    tMember = member(getResourceManager().createMember(me.getID() & "_" & tid, #bitmap))
    tSprite = sprite(getSpriteManager().reserveSprite(me.getID()))
    tSprData = [:]
    tMemberList[tid] = tMember
    tSpriteList[tid] = tSprite
    tSpriteData[tid] = tSprData
  ...
  if tGroupNum = 0 then
    me.pLocX = me.pLocX + tGroupData[#rect][1]
    me.pLocY = me.pLocY + tGroupData[#rect][2]
    me.pWidth = tGroupData[#rect].width
    me.pHeight = tGroupData[#rect].height
  else
    tNewW = pClientRect[1] + pClientRect[3] + tGroupData[#rect].width
    tNewH = pClientRect[2] + pClientRect[4] + tGroupData[#rect].height
    if tNewW <> me.pWidth or tNewH <> me.pHeight then
      me.resizeTo(tNewW, tNewH)
    end if
  end if
  pClientRect = pClientRect + tGroupData[#border]
  repeat with i = 1 to tSpriteList.count
    tSpriteData[i][#loc] = tSpriteList[i].loc - [tGroupData[#rect][1], tGroupData[#rect][2]]
    tSpriteList[i].loc = point(me.pLocX, me.pLocY) + tSpriteData[i][#loc]
    tid = tMemberList.getPropAt(i)
    pMemberList.addProp(tid, tMemberList[tid])
    me.pSpriteList.addProp(tid, tSpriteList[tid])
  end repeat
...
on prepare me
  ...
  if me.pWidth + tOffX < 64 then
    tOffX = 64 - me.pWidth
  end if
  if me.pHeight + tOffY < 64 then
    tOffY = 64 - me.pHeight
  end if
