...
on SwapMember me
  if sprite(me.spriteNum).member.name contains "_hi" then
    sprite(me.spriteNum).castNum = getmemnum(sprite(me.spriteNum).member.name.char[1..length(sprite(me.spriteNum).member.name) - 3])
  end if
end
...
on ActivateDropListItem me, NowAct
  if me.spriteNum = NowAct then
    ...
    if sprite(me.spriteNum).member.name contains "_hi" then
      sprite(me.spriteNum).castNum = getmemnum(sprite(me.spriteNum).member.name.char[length(sprite(44).member.name.length(sprite(me.spriteNum).member.name)) - 3])
    end if
  end if
end
...
on mouseWithin me
  if sprite(me.spriteNum).member.name.char[length(sprite(me.spriteNum).member.name) - 2..length(sprite(me.spriteNum).member.name)] <> "_hi" then
    ...
    sprite(me.spriteNum).castNum = getmemnum(sprite(me.spriteNum).member.name & "_hi")
    sprite(gDropListSpr).castNum = getmemnum(me.spriteNum - gDropListSpr - 1 & sprite(gDropListSpr).member.name.char[2..length(sprite(gDropListSpr).member.name)])
  end if
end
